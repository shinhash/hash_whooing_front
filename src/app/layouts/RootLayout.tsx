...
export function RootLayout() {
  ...
  const NAV = [
    ...
    { id: "transactions", label: "거래내역", icon: ListIcon },
    { id: "journal", label: "분개 입력", icon: PenIcon },
    { id: "accounts", label: "계정과목", icon: BookIcon },
    { id: "reports", label: "보고서", icon: ChartIcon },
  ];
  ...
  return (
    <div className="flex h-full min-h-screen overflow-hidden bg-[#0f1117]">
      ...
      <aside 
        ...
        <div className="mx-3 mt-4 rounded-lg border border-[#2a3348] bg-[#1e2535] px-3 py-2.5">
          <p className="text-[10px] font-medium uppercase tracking-widest text-[#64748b]">
            <Link to="/ledgers" className="app-header__brand">가계부</Link>
          </p>
          <p className="mt-0.5 text-sm font-medium text-[#e2e8f0]">홍길동 가계</p>
      ...
      {/* <header className="app-header">
        <Link to="/ledgers" className="app-header__brand">
          가계부
        </Link>
        <nav className="app-header__nav">
          <Link to="/ledgers">내 장부</Link>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-[#2a3348] bg-[#161b27] px-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="hidden text-[11px] text-[#64748b] sm:block">2026년 8월 24일 일요일</span>
            <button className="rounded-lg bg-[#2dd4bf] px-3 py-1.5 text-xs font-semibold text-[#0f1117] transition-opacity hover:opacity-90">+ 거래 추가</button>
          </div>
        </header>

        <main className="app-content">
          {/* 하위 라우트(ledgers, settings 등)가 여기에 렌더링된다 */}
          <Outlet />
        </main>
      </div>
    </div>
